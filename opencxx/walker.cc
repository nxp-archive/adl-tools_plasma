...
char* Walker::default_template_metaclass = nil;
					   ...
					   Ptree* class_def)
{
    ...
    if(metaobject == nil && default_template_metaclass != nil){
	metaobject
	    = opcxx_ListOfMetaclass::New(default_template_metaclass, def, nil);
	if(metaobject == nil)
	    MopErrorMessage2(
			"the default template metaclass cannot be loaded: ",
...
Ptree* Walker::TranslateClassSpec(Ptree* spec)
{
    ...
    Class* metaobject = nil;
    if(Ptree::Length(class_def) == 4) // "class", classname, inheritance, body
...
Ptree* Walker::TranslateUserPlain(Ptree* exp)
{
    return exp;
}
...
{
    Typeof(exp->First(), t); // :TODO: implement correct type resolution for user-defined operators
}
...
{
  // The type is the type of the child expression.
  exp->Second()->Typeof(this,t);
}
