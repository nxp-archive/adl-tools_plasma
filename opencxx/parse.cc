...
/*
  type.specifier
  : {cv.qualify} (integral.or.class.spec | name | typeof) {cv.qualify}
*/
...
bool Parser::rTempArgDeclaration(Ptree*& decl)
{
  ...
  if(t0 == CLASS && (lex->LookAhead(1) == ',' || lex->LookAhead(1) == '>')) {
    // Placeholder argument.
    lex->GetToken(tk1);
    decl = Ptree::List(new Leaf(tk1));
  }
  else if(t0 == CLASS && lex->LookAhead(1) == Identifier){
	lex->GetToken(tk1);
	lex->GetToken(tk2);
...
bool Parser::rTemplateArgs(Ptree*& temp_args, Encoding& encode)
{
	...
	char* pos = lex->Save();
	type_encode.Clear();
    if(rLogicalOrTypeExpr(a, type_encode))
...
/*
  logical.or.typeexpr
  : logical.and.typeexpr
  | logical.or.typexpr LogOrOp logical.and.typexpr
*/
bool Parser::rLogicalOrTypeExpr(Ptree* &exp,Encoding& type_encode)
{
  Token tk;
  Ptree *right;
  Encoding encode;

  if (!rLogicalAndTypeExpr(exp,type_encode)) {
    return FALSE;
  }
  while (lex->LookAhead(0) == LogOrOp) {
    lex->GetToken(tk);
    // If we have any logicals, it's a boolean.
    type_encode.Clear();
    type_encode.Append('b');
    encode.Clear();
    if (!rLogicalAndTypeExpr(right,encode)) {
      return FALSE;
    }
    exp = new PtreeInfixExpr(exp,Ptree::List(new Leaf(tk),right));
  }
...
/*
  logical.and.typeexpr
  : logical.unary.typeexpr
  | logical.and.typexpr LogAndOp logical.unary.typexpr
*/
bool Parser::rLogicalAndTypeExpr(Ptree* &exp,Encoding& type_encode)
{
  Token tk;
  Ptree *right;
  Encoding encode;

  if (!rLogicalUnaryTypeExpr(exp,type_encode)) {
    return FALSE;
  }
  while (lex->LookAhead(0) == LogAndOp) {
    lex->GetToken(tk);
    ...
    type_encode.Append('b');
    encode.Clear();
    if (!rLogicalUnaryTypeExpr(right,encode)) {
      return FALSE;
    }
    exp = new PtreeInfixExpr(exp,Ptree::List(new Leaf(tk),right));
  }
...
/*
  ...
  | '!' typename
*/
bool Parser::rLogicalUnaryTypeExpr(Ptree* &exp,Encoding& type_encode)
{
  Token tk;
  Ptree *right;
  Encoding encode;
  ...
  if (lex->LookAhead(0) == '!') {
    lex->GetToken(tk);
    if (!rTypeName(right,encode)) {
      return FALSE;
    }
    type_encode.Clear();
    type_encode.Append('b');    
    exp = new PtreeUnaryExpr(new Leaf(tk),
                              Ptree::List(right));
    return TRUE;
  } else {
    return rTypeName(exp,type_encode);
  }
}
...
bool Parser::optTypeofExpr(Ptree*& exp)
{
    ...
    // Do not consume the token if we do not
    // have a typeof statement.
...
/*
  userdef.block
  : UserKeyword6 compound.statement
*/
bool Parser::rUserdefBlock(Ptree*& st)
{
  Token tk;
  Ptree *keyword, *body;
...
bool Parser::rStatement(Ptree*& st)
{
    ...
    case UserKeyword6:
      ...
      st = new PtreeUserPlainStatement(exp->First(),exp->Rest());
      return TRUE;
    case UserKeyword:
    case UserKeyword2:
    case UserKeyword3:
