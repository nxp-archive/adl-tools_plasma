...
extern lt_dlhandle LoadSoLib(const char* file_name);
extern void* LookupSymbol(lt_dlhandle handle, const char* symbol);
	// :TODO: move the above to header
...
void Metaclass::Load(char* mcn, int len)
{
#if USE_DLOADER
#if USE_SO
  ...
  string file_name = metaclass_name + "-init.so";

  lt_dlhandle handle_init = LoadSoLib(file_name.c_str()); // load <metaclass>-init.so
  ...
  file_name = metaclass_name + ".so";
  lt_dlhandle handle = LoadSoLib(file_name.c_str());	// load <metaclass>.so

  if (verboseMode) cerr << "Initialize.. ";

  // call opcxx_init_<metaclass>() in <metaclass>.so

  func_name = generate_symbol("opcxx_init", metaclass_name.c_str());
  void (*func)();
  func = (void (*)())LookupSymbol(handle, func_name.c_str());
  (*func)();

  if(verboseMode) cerr << "Done.\n";
  ...
  // previous code
    ...
    memmove(func_name + sizeof(header) - 1, mcn, len);
    ...
    memmove(file_name, mcn, len);

    strcpy(file_name + len, "-init.so");
    lt_dlhandle handle_init = LoadSoLib(file_name);	// load <metaclass>-init.so
    ...
    lt_dlhandle handle = LoadSoLib(file_name);	// load <metaclass>.so
    ...
    lt_dlhandle handle = LoadSoLib(file_name);	// load <metaclass>.dll
...
lt_dlhandle Metaclass::LoadSoLib(const char* file_name)
{
    ...
    handle = OPENCXX_NAMESPACE::LoadSoLib(file_name);
#endif /* USE_DLOADER */
...
void* Metaclass::LookupSymbol(lt_dlhandle handle, const char* symbol)
{
    void* func = 0;
#if USE_DLOADER
    func = OPENCXX_NAMESPACE::LookupSymbol(handle, symbol);
#endif
    return func;
}
