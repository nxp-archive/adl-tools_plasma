 ...
 *
 *  Returns @c nil for non-object types.
 */
...
Class* ClassWalker::MakeMetaobjectForPlain() {
    if (Class::for_plain == nil) {
	Encoding encode;
	Ptree* name = new Leaf("<Plain>", 7);
	encode.SimpleName(name);
	Ptree* class_def
	    = new PtreeClassSpec(Class::class_t,
				 Ptree::List(name, nil,
	...
	Class* metaobject = opcxx_ListOfMetaclass::New(
			Class::metaclass_for_plain,
			class_def, nil);
	if(metaobject == nil)
	    MopErrorMessage2(
		"the metaclass for plain constructs cannot be loaded: ",
		Class::metaclass_for_plain);

	metaobject->SetEnvironment(env);
	Class::for_plain = metaobject;
    }

    return Class::for_plain;
}
...
Ptree* ClassWalker::TranslateFuncall(Ptree* exp)
{
	...
	else if (Class::for_c_functions != nil) {
...
Ptree* ClassWalker::TranslateUserPlain(Ptree* exp)
{
    TypeInfo type;
    Ptree* keyword = exp->First();
    Ptree* rest = exp->Rest();

    Class* metaobject = nil;
    if (Class::metaclass_for_plain != nil)
        metaobject = MakeMetaobjectForPlain();

    if(metaobject != nil){
        NewScope();
        Ptree* exp2 = metaobject->TranslateUserPlain(env, keyword, rest);
        ExitScope();
        return exp2;
    }

    ErrorMessage("No metaobject installed for simple user blocks.",keyword,keyword);
    return nil;
}
