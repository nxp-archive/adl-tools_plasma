...
static char thisVersion[] = "2.5.6_testing";
...
void ChangeCppOption(int arg,const char* value);
void ChangeCcOption(int arg,const char* value);
void CloseCcOptions();
...
void ParseCmdOptions(int from, int argc, char** argv, char*& source_file)
{
  ...
  for(i = from; i < argc; ++i)
    if(strcmp("--", argv[i]) == 0)
      ...
      regularCpp = TRUE;
    else if (strncmp("--comp=", argv[i],7) == 0) {
      // Override default compiler name.  Point to new string-
      // should be okay, since ARGV data is constant.
      compilerName = &argv[i][7];
      if (! *compilerName ) {
        cerr << "Error:  The --comp option must be given a value.\n\n";
        ShowHelp(argv);
        exit (1);
      }
      // Change arg0 of cpp and cc options.
      ChangeCppOption(0,compilerName);
      ChangeCcOption(0,compilerName);
    }
    else if(*argv[i] == '-'
...
static void ShowHelp(char** argv)
{
    cerr << "Usage: "
	 << argv[0]
	 << " [-w][-l][-s][-V][-v][-E][-m[<file name>]][-u<.ext>][-c][-n][-p][--regular-c++]\n"
	 << "\t\t[-I<directory>][-D<macro>[=<def>]][-M<option>[=<value>]][--comp=<compiler]\n"
	 ...
	 << "    -w             Enable wide characters support\n"
         << "    -u<ext>        Specify a source file extension, e.g. \".foo\".\n"
	 << "\n"
	 ...
	 << "\n"
	 << " Other options\n"
         << "    --comp=<compiler> Specify <compiler> as the compiler to be used.\n"
...
void ChangeCppOption(int arg,const char *value)
{
  if (arg < 0 || arg >= cppArgc) {
    cerr << "OpenC++:  Bad cpp argument change (item " << arg << ")" << endl;
    exit (1);
  }
  cppArgv[arg] = value;
}
...
{
  if (arg < 0 || arg >= ccArgc) {
    cerr << "OpenC++:  Bad cc argument change (item " << arg << ")" << endl;
    exit (1);
  }
  ccArgv[arg] = value;
}
...
bool IsCxxSource(char* fname)
{
    ...
    if (user_ext && !strcmp(ext,user_ext))
      return TRUE;
