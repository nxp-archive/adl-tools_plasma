...
char* Class::metaclass_for_plain = nil;
Class* Class::for_plain = nil;
...
Ptree* Class::TranslateFunctionCall(Environment* env, Ptree* object,
				    Ptree* arglist)
{
    return new PtreeFuncallExpr
(TranslateExpression(env, object),
				TranslateArguments(env, arglist));
}
...
Ptree* Class::TranslateUserPlain(Environment* env,Ptree* keyword, Ptree*)
{
    ErrorMessage(env, "unsupported user statement: ", keyword, keyword);
    return nil;
}
...
void Class::RegisterNewBlockStatement(char* str)
{
	...
	MopErrorMessage("Class::RegisterNewBlockStatement()",
			"the keyword is already used.");
}
